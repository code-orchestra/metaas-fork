...
/**
 * This tests checks if the float literal is parsed correctly.
 *
...
public class FloatLiteralTest extends TestCase {
	public void testIt() {
        ...
        assertTrue(assignmentExpression.getRightSubexpression() instanceof ASFloatLiteral);
        ASFloatLiteral floatLiteral = (ASFloatLiteral) assignmentExpression.getRightSubexpression();
        assertEquals("1e-10", floatLiteral.getValue());                 
	}
	...
	private ASCompilationUnit loadSyntaxExample(ActionScriptFactory fact) {
		InputStream in = getClass().getClassLoader().getResourceAsStream("FloatLiteral.as");
