 ...
 */
public class ContinueStatementTest extends TestCase {
	public void testIt() {
        ...
        assertTrue(statement instanceof ASContinueStatement);

        ASContinueStatement continueStatement = (ASContinueStatement) statement;

        assertEquals("myLabel", continueStatement.getLabel());
	}
	...
	private ASCompilationUnit loadSyntaxExample(ActionScriptFactory fact) {
		InputStream in = getClass().getClassLoader().getResourceAsStream("ContinueStatement.as");
